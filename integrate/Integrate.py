...
from Algorithms.DesignToolAlgorithmV4_1D.EXTRAS.join_blocks import JointBlock
from Algorithms.DesignToolAlgorithmV4_1D.BoundaryConditions.FormBoundaryConditionInformation import FormBoundaryConditionInformation
class Integrate():
    def __init__(self, mesh, cfl_flag, t_current, current_step) -> None:
        ...
        self.addBoundaryConditions(BCs = self.mesh.boundary_conditions)
        ...
        if not cfl_flag[0]: #Using fixed dt, cfl_flag has form [False, float dt_fixed]
            ...
            if cfl_flag[1] == "no_ramping":
                ...
                if dt_ramped >= min_dt_from_cfl:
                    ...
                    print("step = ", current_step, " ramped dt used: ", dt_ramped, " cfl dt: ", min_dt_from_cfl)
        ...
        for ind, interface in enumerate(self.mesh.interface_array):
        ...
        self.mesh.cell_array = self.mesh.cell_array[:-1 * self.totalGhostCells]
        self.mesh.interface_array = self.mesh.interface_array[:-1 * self.totalGhostCells]
    ...
    def addBoundaryConditions(self, BCs):
        ...
        for boundary_condition in BCs:
            self.totalGhostCells += boundary_condition[1][1]
            if boundary_condition[1][0] == "ConstantFlux_BC":
                pass
            else:
                ghostCellLayer = FormBoundaryConditionInformation(mesh = self.mesh, BC = boundary_condition).ghostCellLayer
                self.mesh = JointBlock(mesh_object_1 = self.mesh, mesh_object_2 = ghostCellLayer, \
