...
from Algorithms.DT_1D_V4.models.\
        single_phase_multi_species_reactive_pipe_with_tanh_fast_chemistry_for_goal_massf_profile.\
            single_phase_multi_species_reactive_pipe_with_fast_chemistry_for_goal_massf_cell \
                import SinglePhaseMultiSpeciesReactivePipeWithFastChemistryForGoalMassfCell

class SinglePhaseMultiSpeciesReactivePipeWithTanhFastChemistryForGoalMassfProfile(SingleInlet1DMeshObject):
    def __init__(self, n_cells, comp_label, geometry, init_flow_state, goal_massf_profile, \
        ...
        self.initialise_cells(geometry = geometry, \
                                ...
                                n_cells = n_cells, \
                                goal_massf_profile = goal_massf_profile)
    ...
    def initialise_cells(self, geometry, init_flow_state, comp_label, n_cells, goal_massf_profile):
        [D, L] = geometry
        [sp_name, inlet_massf_value, equil_massf, x, reaction, alpha, x_half] = goal_massf_profile
        equilibrium_fraction_list = [None] * n_cells
        pos_x_list = [None] * n_cells
        goal_massf_list = [None] * n_cells
        for cell in range(n_cells):
            ...
            cell_object = SinglePhaseMultiSpeciesReactivePipeWithFastChemistryForGoalMassfCell(cell_id = cell, \
                                                                                    label = comp_label)
            ...
            equilibrium_fraction = 0.5 * (1.0 + np.tanh(alpha / L * (pos_x - x_half)))
            equilibrium_fraction_list[cell] = equilibrium_fraction
            pos_x_list[cell] = pos_x
            goal_massf = inlet_massf_value + equilibrium_fraction * (equil_massf - inlet_massf_value)
            goal_massf_list[cell] = goal_massf
            cell_object.combustion_parameters = [sp_name, goal_massf, x, reaction]
            cell_object.initialise_conserved_quantities()            
            self.cell_array[cell] = cell_object
        #print("x=", pos_x_list)   
        #print("y1=", equilibrium_fraction_list)
        #print("y2=", goal_massf_list)
