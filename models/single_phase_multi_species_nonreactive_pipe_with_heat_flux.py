...
from Algorithms.DT_1D_V4.models.single_phase_multi_species_nonreactive_pipe_with_heat_flux_cell \
                    import SinglePhaseMultiSpeciesNonReactivePipeWithHeatFluxCell

class SinglePhaseMultiSpeciesNonReactivePipeWithHeatFlux(SingleInlet1DMeshObject):
    def __init__(self, n_cells, geometry, init_flow_state, \
                recon_props, recon_scheme, limiter, update_from, comp_label,\
                flux_scheme, heat_flux, reverse_direction_for_ghost_cells = False) -> None:
    ...
    def initialise_cells(self, geometry, init_flow_state, comp_label, n_cells, heat_flux):
        [D, L] = geometry
        ...
        for cell in range(n_cells):
            ...
            cell_object = SinglePhaseMultiSpeciesNonReactivePipeWithHeatFluxCell(cell_id = cell, \
                                                                    label = comp_label, heat_flux = heat_flux)
            geo = {
                "dx"    :   L / n_cells,
